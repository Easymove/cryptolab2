...
namespace cryptolab2
{
    class DESutils
    {
        ...
        public void EncryptFile(string inputFileName, string outputFileName)
        {
            try
            {
                var outFile = File.Open(outputFileName, FileMode.OpenOrCreate);
                var data = File.ReadAllBytes(inputFileName);
                var encryptedData = EncryptBytes(data, false);
                outFile.Write(encryptedData, 0, encryptedData.Length);
                outFile.Close();
            }
            catch (IOException e)
            {
                _output.AppendText(string.Format("IO exception occured: {0}", e.Message));
            }
            catch (CryptographicException e)
            {
                _output.AppendText(string.Format("A Cryptographic error occurred: {0}\n", e.Message));
            }
        ...
        public void DecryptFile(string inputFileName, string outputFileName)
        {
            try
            {
                var outFile = File.Open(outputFileName, FileMode.OpenOrCreate);
                var data = File.ReadAllBytes(inputFileName);
                var decryptedData = DecryptBytes(data, false);
                outFile.Write(decryptedData, 0, decryptedData.Length);
                outFile.Close();
            }
            catch (IOException e)
            {
